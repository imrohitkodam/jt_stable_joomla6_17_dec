			...
			<table class="table table-bordered table-striped">
				...
				<thead>
					<tr>
						...
						<th><?php echo Text::_('COM_JTICKETING_COUPONS_APPLYON'); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php
						...
						{
							$assignedEventIds = is_array($coupon->event_ids) ? $coupon->event_ids : explode(',', $coupon->event_ids);
							if (in_array($this->event->integrationId, $assignedEventIds))
							{
								// Safely retrieve values from the coupon object
								$name = htmlspecialchars($coupon->name ?? 'N/A', ENT_QUOTES, 'UTF-8');
								$code = htmlspecialchars($coupon->code ?? 'N/A', ENT_QUOTES, 'UTF-8');
								$groupDiscount = (int) ($coupon->group_discount_tickets ?? 0);
								$value = $coupon->val_type == 1 ? ($coupon->value ?? 0) . ' %' : $this->utilities->getFormattedPrice($coupon->value ?? 0, false);
								?>
								<tr>
									<td><?php echo $name; ?></td>
									<td><?php echo $code; ?></td>
									<td><?php echo $value; ?></td>
									<td><?php echo $groupDiscount . ' tickets'; ?></td>
								</tr>
					<?php
							}
						}
	...
	<div class="row eventDetails-body">
		...
		<div class="col-xs-12 af-mt-30 order-sm-3">
			...
			<?php
				...
				{
					$address = $this->item->venue > 0 ? $this->venueAddress : (!empty($this->item->location) ? $this->item->location: $this->item->event_address);
				?>
